...
const url = "https://veterinaria-backend-nine.vercel.app/duenos";
let duenos = [];

async function listarDuenos() {
  try {
    const respuesta = await fetch(url);
		const duenosDelServidor = await respuesta.json();
		if (Array.isArray(duenosDelServidor)) {
			duenos = duenosDelServidor;
		}
    if (duenos.length > 0) {
